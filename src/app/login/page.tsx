...
function LoginPageContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Authentication error - user will see message in UI, no console logging needed
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center transition-colors duration-300 ${isDark ? 'bg-gray-900' : 'bg-gradient-to-br from-gray-50 to-gray-100'
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className={`absolute -top-40 -right-40 w-80 h-80 rounded-full blur-3xl opacity-20 ${isDark ? 'bg-red-600' : 'bg-red-500'
          }`}></div>
        <div className={`absolute -bottom-40 -left-40 w-80 h-80 rounded-full blur-3xl opacity-20 ${isDark ? 'bg-green-600' : 'bg-green-500'
          }`}></div>
      </div>

      <div className="relative z-10 w-full max-w-md px-6">
        <div className={`rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ${isDark ? 'bg-gray-800 shadow-gray-950/50' : 'bg-white shadow-gray-500/30'
          }`}>
          {/* Header */}
          <div className={`px-8 pt-8 pb-6 text-center border-b ${isDark ? 'border-gray-700' : 'border-gray-200'
            }`}>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                autoComplete="email"
                className={`w-full px-4 py-3 rounded-lg border transition-all duration-200 focus:outline-none focus:ring-2 disabled:opacity-50 ${isDark
            ...
            <button
              ...
              className={`w-full py-3 px-4 rounded-lg font-semibold transition-all duration-300 transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none ${isDark
            ...
            <div className="text-center">
              <Link
                href="/"
                className={`text-sm hover:underline transition-colors ${isDark ? 'text-gray-400 hover:text-gray-300' : 'text-gray-600 hover:text-gray-700'
                  }`}
              >
                ← Back to website
              </Link>
