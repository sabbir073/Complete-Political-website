...
export async function POST(request: NextRequest) {
  try {
    ...
    const s3Key = generateVolunteerS3Key(finalFilename);

    // Upload to S3
    const result = await uploadToS3(fileBuffer, s3Key, chunkData.metadata.fileType);
    ...
    return NextResponse.json({
      ...
      fileType: chunkData.metadata.fileType,
