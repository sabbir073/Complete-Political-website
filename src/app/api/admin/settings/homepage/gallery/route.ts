...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = supabase.from('settings').select(includeTranslations ? `*, setting_translations(language_code, translated_value)` : '*').eq('category', 'home').eq('subcategory', 'gallery').eq('is_active', true).order('display_order', { ascending: true });
    const { data: settings, error } = await query;
    if (error) return NextResponse.json({ error: 'Failed to fetch gallery settings' }, { status: 500 });
    ...
    return NextResponse.json({ success: true, data: { category: 'home', subcategory: 'gallery', settings: processedSettings, grouped: groupedSettings, count: formattedSettings.length } });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: settings, error: fetchError } = await supabase.from('settings').select('id, setting_key, setting_value, default_value').eq('category', 'home').eq('subcategory', 'gallery').eq('is_active', true);
    ...
    for (const setting of settings || []) {
      try {
        await supabase.from('setting_history').insert({ setting_id: setting.id, old_value: setting.setting_value, new_value: setting.default_value, changed_by: user.id, change_reason: 'Reset gallery section to defaults' });
    ...
    return NextResponse.json({ success: resetErrors.length === 0, message: 'Reset gallery section settings to defaults', data: { category: 'home', subcategory: 'gallery', reset_count: resetResults.length, reset_settings: resetResults, errors: resetErrors } });
  } catch (error) {
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}