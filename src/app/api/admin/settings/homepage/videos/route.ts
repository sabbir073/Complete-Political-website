...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, data: { category: 'home', subcategory: 'video', settings: processedSettings, grouped: groupedSettings, count: formattedSettings.length } });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: settings, error: fetchError } = await supabase.from('settings').select('id, setting_key, setting_value, default_value').eq('category', 'home').eq('subcategory', 'video').eq('is_active', true);
    ...
    for (const setting of settings || []) {
      try {
        await supabase.from('setting_history').insert({ setting_id: setting.id, old_value: setting.setting_value, new_value: setting.default_value, changed_by: user.id, change_reason: 'Reset video section to defaults' });
    ...
    return NextResponse.json({ success: resetErrors.length === 0, message: 'Reset video section settings to defaults', data: { category: 'home', subcategory: 'video', reset_count: resetResults.length, reset_settings: resetResults, errors: resetErrors } });
  } catch (error) {
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}