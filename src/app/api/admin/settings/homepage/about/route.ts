...
export async function GET(request: NextRequest) {
  try {
    ...
    const query = supabase.from('settings').select(includeTranslations ? `*, setting_translations(language_code, translated_value)` : '*').eq('category', 'home').eq('subcategory', 'about').eq('is_active', true).order('display_order', { ascending: true });
