...
export default function BlogSettingsPage() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch('/api/admin/settings/homepage/blog?translations=true');
      ...
      if (data.success) {
        ...
        toast.error('Failed to load blog section settings');
      }
    } catch (error) {
  ...
  const handleReset = async () => {
    const result = await showConfirm(
      'Reset to Defaults?',
      'Are you sure you want to reset all blog section settings to their default values? This action cannot be undone.',
      'Yes, Reset',
      'Cancel'
    );
    ...
    try {
      const response = await fetch('/api/admin/settings/homepage/blog', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        showSuccess('Settings Reset!', 'Blog section settings have been reset to their default values.');
        setHasChanges(false);
        setChangedFields(new Set());
        fetchSettings();
      } else {
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Blog Section Settings</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            Configure the blog section including title, description, and display options for blog posts.
          </p>
        </div>
