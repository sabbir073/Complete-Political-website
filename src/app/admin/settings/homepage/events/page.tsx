...
export default function EventsSettingsPage() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch('/api/admin/settings/homepage/events?translations=true');
      ...
      if (data.success) {
        ...
        toast.error('Failed to load events section settings');
      }
    } catch (error) {
  ...
  const handleReset = async () => {
    const result = await showConfirm(
      'Reset to Defaults?',
      'Are you sure you want to reset all events section settings to their default values? This action cannot be undone.',
      'Yes, Reset',
      'Cancel'
    );
    ...
    try {
      const response = await fetch('/api/admin/settings/homepage/events', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        showSuccess('Settings Reset!', 'Events section settings have been reset to their default values.');
        setHasChanges(false);
        setChangedFields(new Set());
        fetchSettings();
      } else {
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Events Section Settings</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            Configure the events section including title, description, and display options.
          </p>
        </div>
