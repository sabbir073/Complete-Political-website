...
export default function AboutSettingsPage() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch('/api/admin/settings/homepage/about?translations=true');
      ...
      if (data.success) {
        ...
        toast.error('Failed to load about section settings');
      }
    } catch (error) {
  ...
  const handleReset = async () => {
    const result = await showConfirm(
      'Reset to Defaults?',
      'Are you sure you want to reset all about section settings to their default values? This action cannot be undone.',
      'Yes, Reset',
      'Cancel'
    );
    ...
    try {
      const response = await fetch('/api/admin/settings/homepage/about', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        showSuccess('Settings Reset!', 'About section settings have been reset to their default values.');
        setHasChanges(false);
        setChangedFields(new Set());
        fetchSettings();
      } else {
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">About Section Settings</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            Configure the about section including title, description, images, and layout options.
          </p>
        </div>
