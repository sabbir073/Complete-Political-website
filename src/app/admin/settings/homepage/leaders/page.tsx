...
export default function LeadersSettingsPage() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch('/api/admin/settings/homepage/leaders?translations=true');
      ...
      if (data.success) {
        ...
        toast.error('Failed to load great leaders settings');
      }
    } catch (error) {
  ...
  const handleReset = async () => {
    const result = await showConfirm(
      'Reset to Defaults?',
      'Are you sure you want to reset all great leaders settings to their default values? This action cannot be undone.',
      'Yes, Reset',
      'Cancel'
    );
    ...
    try {
      const response = await fetch('/api/admin/settings/homepage/leaders', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        showSuccess('Settings Reset!', 'Great leaders settings have been reset to their default values.');
        setHasChanges(false);
        setChangedFields(new Set());
        fetchSettings();
      } else {
  ...
  const getInputComponent = (setting: Setting) => {
    ...
    if (setting.setting_type === 'image' || (setting.setting_key.includes('image') && setting.setting_type !== 'boolean')) {
      return (
        <MediaPicker
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Great Leaders Settings</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-2">
            Configure the great leaders section including title, description, and display settings.
          </p>
        </div>
