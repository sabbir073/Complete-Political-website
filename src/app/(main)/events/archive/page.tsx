...
export default function PastEventsPage() {
    ...
    const fetchEvents = async (pageNum: number, isLoadMore: boolean = false) => {
        try {
            ...
            const res = await fetch(`/api/events?page=${pageNum}&limit=${EVENTS_PER_PAGE}&filter=past`);
    ...
    return (
        <div className={`min-h-screen ${isDark ? 'bg-gray-900' : 'bg-gray-50'}`}>
            {/* Hero Header */}
            <div className={`relative py-16 md:py-24 ${isDark ? 'bg-gray-800' : 'bg-gradient-to-r from-gray-600 to-gray-700'}`}>
                ...
                <div className="relative max-w-7xl mx-auto px-6 md:px-16 text-center">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur-sm text-white text-sm font-medium mb-4">
                        ...
                        {language === 'bn' ? 'অতীত ইভেন্ট' : 'Past Events'}
                    </div>
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
                        {language === 'bn' ? 'অতীত ইভেন্ট সমূহ' : 'Past Events'}
                    </h1>
                    <p className="text-lg md:text-xl text-white/90 max-w-2xl mx-auto">
                        {language === 'bn'
                            ? 'আমাদের সম্পন্ন হওয়া সকল অনুষ্ঠান ও কার্যক্রমের সংরক্ষণাগার'
                            : 'Archive of all our completed events and activities'
                        }
                    </p>
                </div>
            </div>
                ...
                {!loading && events.length === 0 && (
                    <div className={`text-center py-16 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                        ...
                        <h3 className={`text-xl font-semibold mb-2 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                            {language === 'bn' ? 'কোনো অতীত ইভেন্ট নেই' : 'No Past Events'}
                        </h3>
                        <p className="mb-6">
                            {language === 'bn'
                                ? 'এখনো কোনো ইভেন্ট সম্পন্ন হয়নি।'
                                    ...
                                    <div className={`h-full rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:scale-[1.02] ${
                                        ...
                                        <div className="relative h-56 overflow-hidden">
                                            ...
                                            <div className="absolute top-4 left-4">
                                                <span className={`px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wide ${isDark ? 'bg-gray-600' : 'bg-gray-500'} text-white`}>
