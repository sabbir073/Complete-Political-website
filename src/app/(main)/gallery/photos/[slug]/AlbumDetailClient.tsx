...
export default function AlbumDetailClient({ slug, initialAlbum }: AlbumDetailClientProps) {
    ...
    // Fetch album if not provided initially
    useEffect(() => {
    ...
    // Share URL and data
    const shareUrl = `${siteConfig.url}/gallery/photos/${slug}`;
    const shareTitle = album ? getText(album.name_en, album.name_bn) : 'Photo Album';
    const shareDescription = album ? (getText(album.description_en, album.description_bn) || `Browse ${album.photos?.length || 0} photos`) : '';
                        ...
                        {album.photos && album.photos.length > 0 && (
                            <div className={`mt-8 p-6 rounded-2xl ${isDark ? 'bg-gray-800' : 'bg-white shadow-lg'}`}>
                                <h3 className={`text-lg font-bold mb-4 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                                    {language === 'bn' ? 'এই অ্যালবাম শেয়ার করুন' : 'Share This Album'}
                                </h3>
                                <SocialShare
                                    url={shareUrl}
                                    title={shareTitle}
                                    description={shareDescription}
                                    image={album.cover_image || album.photos[0]?.image_url}
                                    variant="buttons"
                                    size="sm"
                                    showLabel={true}
                                />
